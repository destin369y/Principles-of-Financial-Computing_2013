...
double R_Bb;
double v11_Bb,v12_Bb,v21_Bb,v22_Bb,v31_Bb,v32_Bb;
double v3_Bb,log_diff;
double prob_adjustment,prob_adjustment_anti;
...
int main()
{
	...
	cin >> m;
	cout << "type 0 if not use the Brownian bridge method," << endl;
	cout << "type 1 if use the Brownian bridge method:" << endl;	
	cin >> index;
	...
	switch(index)
	{
		...
		case 1:
			for(int j=0;j<num_path;j++)
			{
				...
				Normal=Normal1*P[1][1];
				Sn1=S1*R_Bb*exp(-yield*t)*exp(-v11_Bb+v12_Bb*Normal);
				Sn_anti1=S1*R_Bb*exp(-yield*t)*exp(-v11_Bb-v12_Bb*Normal);
				prob_adjustment = exp(-2*log_diff*log(H/Sn1)/(v3_Bb));
				prob_adjustment_anti = exp(-2*log_diff*log(H/Sn_anti1)/(v3_Bb));

				Normal=Normal1*P[2][1]+Normal2*P[2][2];
				Sn2=S2*R_Bb*exp(-yield*t)*exp(-v21_Bb+v22_Bb*Normal);
				Sn_anti2=S2*R_Bb*exp(-yield*t)*exp(-v21_Bb-v22_Bb*Normal);

				Normal=Normal1*P[3][1]+Normal2*P[3][2]+Normal3*P[3][3];
				Sn3=S3*R_Bb*exp(-yield*t)*exp(-v31_Bb+v32_Bb*Normal);
				Sn_anti3=S3*R_Bb*exp(-yield*t)*exp(-v31_Bb-v32_Bb*Normal);

				if (((S<H)&&(Sn1<H))||((S>=H)&&(Sn1>=H)))
					Est+=max(X-(Sn1+Sn2+Sn3)/3,0)*(1-prob_adjustment);
				else	Est+=0; 
				if (((S<H)&&(Sn_anti1<H))||((S>=H)&&(Sn_anti1>=H)))
					Est+=max(X-(Sn_anti1+Sn_anti2+Sn_anti3)/3,0)*(1-prob_adjustment_anti);
				else	Est+=0; 
			}	
			Est=Est/(2*num_path);
			Est=Est/R_Bb;
			result=int((Est)*100000+0.5)/100000.0;
			cout<< "Brownian bridge option value: " << result <<endl;
